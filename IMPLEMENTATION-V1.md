...
| The sidecar can complete an hCaptcha challenge | **Open.** The probe is `browser --cdp http://127.0.0.1:9224 eval "document.createElement('canvas').getContext('webgl') ? 'YES' : 'NO'"`, and it returns `YES` — verified. That is necessary and not sufficient: it passed while hCaptcha still killed the renderer on the real challenge path, so this row stays open until a login behind hCaptcha completes. `HCAPTCHA-FINDINGS.md` holds what has been ruled out, by what method, and the one measurement still missing. |
