...
- **Headless crawling does not work in this image, and it fails quietly.**
  `-hl`/`-hh` download a Chromium build via go-rod from
  `storage.googleapis.com` into `/root/.cache/rod/browser/chromium-<rev>` on
  first use (about 150 MB, needs egress). That Chromium then cannot launch,
  because the image lacks its shared libraries — `ldd` reports 14 missing,
  including `libnss3.so`, `libnssutil3.so`, `libsmime3.so`, `libnspr4.so`,
  `libatk-1.0.so.0`, `libatk-bridge-2.0.so.0`, `libcups.so.2`, `libatspi.so.0`,
  `libXcomposite.so.1`, `libXdamage.so.1`, `libXrandr.so.2`, `libgbm.so.1`.
  The failure mode is the dangerous kind: katana **exits 0** and reports
  `Crawl completed in 1s. 0 endpoints found.` rather than an error. A headless
  crawl that finds nothing has found nothing about the target — it never had a
  browser. Use `-jc` (non-headless JS parsing), which needs no browser and
  works.
