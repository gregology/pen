...
(`ENV PATH="/opt/pen-venv/bin:$PATH"`), and that virtualenv used to carry the
...
If it resolves into `/opt/pen-venv/bin`, use `/usr/local/bin/httpx` explicitly
...
- **Headless screenshots do not work in this image.** `-ss` downloads a
  Chromium build via go-rod into `/root/.cache/rod/browser/`, then fails to
  launch it because the image lacks the browser's shared libraries:
  `Failed to launch the browser ... chrome: error while loading shared
  libraries: libnss3.so: cannot open shared object file`. Needs `libnss3`,
  `libnspr4`, `libatk*`, `libcups`, `libgbm` and friends installed in the
  image. Treat every screenshot flag (`-ss`, `-system-chrome`, `-jsc`,
  `-esb`, `-ehb`) as unavailable until that changes.
