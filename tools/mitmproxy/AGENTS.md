...
| Binaries | `/opt/pen-venv/bin/{mitmdump,mitmproxy,mitmweb}` (virtualenv `/opt/pen-venv`, first on `PATH`) |
...
The venv pins `bcrypt==4.0.1` deliberately: mitmproxy imports `passlib.apache`
at startup, and passlib 1.7.4 calls `bcrypt.hashpw` with a >72-byte test string
during backend detection, which bcrypt 5.x rejects with `ValueError: password
cannot be longer than 72 bytes`. With bcrypt 5.x all three entry points exit 1
before printing anything. Do not upgrade bcrypt in this venv.
