...
Every script is a separate command in `/opt/pen-venv/bin`. They authenticate to
...
| Provenance | Resolved from fortra's git main. The Dockerfile pins `impacket==0.13.1`, but NetExec declares `impacket @ git+https://github.com/fortra/impacket`, so pip replaced it — the installed version is the git revision, not 0.13.1 |
| Location | `/opt/pen-venv/bin/*.py` (virtualenv `/opt/pen-venv`, on `PATH`) |
| Library | `/opt/pen-venv/lib/python3.11/site-packages/impacket` |
...
**Version drift from the Dockerfile.** The image was built expecting impacket
0.13.1; pip resolved 0.14.0.dev0+20260916.40533.c38d1eeb from git because
NetExec requires impacket from git. Flags documented here were read from the
installed scripts, but upstream `main` moves — if a flag is missing, check
`<script>.py -h` before assuming the doc is right.
