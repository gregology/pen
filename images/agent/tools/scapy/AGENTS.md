...
| Location | `/usr/lib/python3/dist-packages/scapy/` (Debian `python3-scapy`), importable from the venv interpreter because `/opt/venvs/scapy` is built with `--system-site-packages` |
| Interpreter | `/opt/venvs/scapy/bin/python3` (3.11.2, first on `PATH`) — plain `python3` is the right one |
