...
Four containers on a Docker network, with deliberately one-directional trust:
...
│  + pentest tools│      └──────────────┘
│                 │      ┌──────────────┐
│  ▲ DevTools     │─────▶│  vpn-gateway │────▶ Internet (via VPN exit node)
│  │ (loopback)   │      └──────────────┘
│  browser        │             ▲
│  (human-solved) │─────────────┘   in the gateway's namespace,
└─────────────────┘                 so its egress is the tunnel too
        ▲
        └── the operator, over the LAN
```
...
| One pinned browser in the agent image, shared by every tool that needs one | Three tools each downloaded their own Chromium at runtime (or failed silently without one), so a screenshot depended on the tunnel being healthy the first time and was lost on every container recreate. One build-time-pinned binary at `/usr/local/bin/chromium` is what go-rod's `LookPath()` resolves for `katana -sc`, `httpx -system-chrome` and `nuclei -sc`, which turns three unpinned downloads into one audited artifact. The rule is scoped to this image: the interactive sidecar runs a different browser from a different provenance — Alpine's packaged Chromium, which is what brings a display and a VNC-servable window — and the two are deliberately not interchangeable. Anything depending on browser identity (fingerprints, `cf_clearance`) is tied to whichever one holds the session. |
| The browser runs with `--no-sandbox`, documented rather than assumed | Chromium aborts at zygote init as root without it and cannot use its namespace sandbox under Docker's default seccomp profile. The renderer that parses a target's JavaScript is therefore not isolated from the agent — but the ProjectDiscovery tools were already in that position via go-rod's `MustDisableSandbox()`, and the barrier that matters is the network topology, not the browser's own sandbox. `tools/browser/sandbox-experiment.sh` tests whether a host permits better; container policy is not weakened to obtain it. |
| An interactive browser sidecar in the gateway namespace, with the agent attached to that same browser over the DevTools protocol | Some of what blocks testing cannot be automated at all — a Cloudflare Managed Challenge, an MFA prompt, a passkey — and Cloudflare documents that a Managed Challenge solved from a different IP than the one it was issued to is not a valid solve. A clearance cookie exported from a home browser is therefore not a substitute for solving the challenge on the exit IP the agent's tools will use. The sidecar shares the gateway namespace, so the session it earns is bound to the same exit node as the agent's traffic and its own egress cannot leak either. The human gets only the view, a LAN control surface like the other UIs; the DevTools endpoint stays on loopback, because it is unauthenticated control of everything that browser holds. |
