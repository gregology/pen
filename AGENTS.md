...
7. **Exit-node rotation happens only through the gateway's control API**, reached at the fixed loopback address `http://localhost:8080` in the shared namespace. It has no credential and does not need one: the kill switch answers port 8080 only from the sandbox network and loopback and drops it from everywhere else, including the tunnel, so reachability is the control. Adding an exit node is a new `.conf` in `/home/user/pen/vpn-configs/`, never a code change or a manual `docker exec` as standard practice.
...
12. **The browser runs unsandboxed, and that is a recorded decision.** Chromium aborts at zygote init as root without `--no-sandbox`, and this container cannot create the user namespaces its namespace sandbox needs (Docker's default seccomp profile; `unshare --user` returns `Operation not permitted`). `tools/browser` therefore passes `--no-sandbox`, so a renderer parsing a target's hostile JavaScript is not sandboxed away from the agent. The ProjectDiscovery tools were already in exactly this position — go-rod's `MustDisableSandbox()` returns true for root on Linux — and what changed is that it is written down instead of implicit. The barrier is the network topology, which is unaffected. `tools/browser/sandbox-experiment.sh` tests whether a dropped-privilege sandboxed browser is possible on a given host; do not weaken container or host policy to obtain one.
13. **Port 8080 is the gateway control API in this namespace, and default proxy settings point at it.** Anything in the agent that defaults to an HTTP proxy on `127.0.0.1:8080` is talking to the VPN gateway's unauthenticated exit-node control surface, not to a proxy. `jwt_tool` is the known case: its generated `jwtconf.ini` writes `proxy = 127.0.0.1:8080` (verified in the pinned source, `jwt_tool.py` line 71) and every live run sends through it unless `-np` is passed. Check any new tool's proxy defaults before its first live run, and pass the disabling flag rather than leaving a default that reaches a control plane.
14. **The interactive browser is the one place a human solves what automation cannot, and its DevTools endpoint is the most dangerous address in the namespace.** The `browser` sidecar shares the gateway namespace, so a challenge solved in it is solved from the exit IP the agent's tools use, and its own traffic cannot leak either. Two consequences follow. CDP is unauthenticated full control of a browser holding live authenticated sessions — it reads its cookies, drives its pages and can exfiltrate through it — so it stays on loopback inside the shared namespace: never a `ports:` entry, never a `socat` forwarder, and nothing in the image may be pointed at it from outside. The image's own CDP forwarder is the trap here — it listens on every interface of the namespace, so the compose file disables it (`CHROMIUM_REMOTE_DEBUGGING=0`) and lets Chromium bind loopback itself on a port the forwarder never used. Do not re-enable it, and do not "helpfully" re-add a forwarder for port 9224. And the profile under `/home/user/pen/browser-profile/` is credential material on the same footing as `vpn-configs/` and `llm-logs/`: gitignored, never committed. The sidecar inherits hard limit 12's position — it runs unsandboxed, and it does not get `SYS_ADMIN`, `--privileged`, or a weakened seccomp profile to obtain one. Because it holds the platform's only interactive login state, it is a target-only browser: Greg's personal browsing and unrelated credentials do not mix into it. Solving a challenge also pins the exit node — rotation invalidates the clearance — so `/switch` and a live interactive session are mutually exclusive.
