...
8. **The DSH GUI is a LAN control surface, not a public service.** Port 3080 is published on the vpn-gateway bound to the host's LAN address only (`10.0.0.10:3080`, matching the agent's `--trusted-host`); other host interfaces stay closed, and remote/public access is a scope change requiring explicit confirmation.
